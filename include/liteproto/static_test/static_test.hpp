...
using s = decltype(std::begin(std::declval<std::map<std::string, int>&>()));
static_assert(std::is_same_v<s, decltype(std::declval<std::map<std::string, int>>().find(std::declval<std::string>()))>);
